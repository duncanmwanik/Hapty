...
String getFormattedTime(DateTime time) {
  int? hour = (time.hour > 12 || time.hour == 0) ? hours24to12[time.hour] : time.hour;
  int minute = time.minute;
  String hour_ = hour.toString().length == 2 ? hour.toString() : '0$hour';
  String minute_ = minute.toString().length == 2 ? minute.toString() : '0$minute';
...
int getCurrentWeekDay() {
  return DateTime.now().weekday == 7 ? 0 : DateTime.now().weekday;
}
